...
const tempArray = [];
class ViewPlayer extends Entity3D {
	...
	constructor(color) {
		...
		this.points = [];
		for (let i = 0; i < 250; i++) {
			this.points.push([0, 0, Math.sin(i / 20) * 0.1]);
		}
	...
	onHeartbeat (hb) {
		...
		if (!isNaN(hb)) {
			if (hb < 50) hb = 50;
			if (hb > 190) hb = 190;
			const hbEase = Circ.easeOut(map(hb, 50, 190, 0, 1));
			hb = map(hbEase, 0, 1, -1, 1);
		}

		if (!hb && hb !== 0) {
			const hbT = Math.random() * 140 + 50;
			
			const hbEase = Circ.easeOut(map(hbT, 50, 190, 0, 1));
			hb = map(hbEase, 0, 1, -1, 1);
		}



		this.heartbeatTarget = hb;// || (Math.floor((Math.random() * 1.5 - 1) * 100) / 100);
	...
	movePoints() {
		...
		for (let i = 0; i < this.points.length; i++) {
			...
			pt[1] = (2 / this.points.length) * i
			// console.log(pt[1], 'pt[1]');
	...
	update() {
		...
		let x = 0;
		let y = 0;
		const freq = CONSTS.hbFreq * 60;

		if (this.hbPoints.length > 1) {
			const hbIndex = (Math.floor(this.time / freq) - 1) * 3;
